...
private:
	...
	std::mt19937 gen = std::mt19937(12345); // for deterministic seeding
...
public:
	...
	void read_graph() ;
	void kCliqueCounting(ui K, std::string alg, double epsilon, ui t, ui refinement_cap) ;
...
private:
	...
	void process_a_subgraph(ui leading_vertex, ui subgraph_n, ui peel_sequence_n, ui *peel_sequence, ui *out_mapping, ui l,
			...
			ui start_idx, std::vector<double> &density_thresholds, ui *ids, double dd) ;
};
