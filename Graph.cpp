...
void Graph::read_graph_binary() {
	...
	ui *degree = new ui[n];
	if(fread(degree, sizeof(int), n, f) != n) {
		printf("WA in reading degrees\n");
		exit(1);
	}
	...
	for(ui i = 0;i < n;i ++) {
		if(degree[i] > 0) {
			if(fread(edges+pstart[i], sizeof(int), degree[i], f) != degree[i]) {
				printf("WA in reading %u neighbors of %u\n", degree[i], i);
				exit(1);
			}
...
void Graph::kCliqueCounting(ui K, string alg, double epsilon, ui sample_n, ui refinement_cap) {
	if(K <= 3 || K >= n) {
	...
	if(m) {
		...
		if(alg == "Pivoter") {
			...
			for(ui i = 0;i < n;i ++) if(pend[peel_sequence[i]]-pstart[peel_sequence[i]] >= K-1) {
				...
				double local_cnt = 0;
				local_cnt = pm->pivotor2(subgraph_n, matrix, degree, rid, ids, K-1);
				total_cnt += local_cnt;
			}
			printf("*** Total number of %u-cliques: %.2lf\n", K, total_cnt);
		}
		else if(alg == "DPColorPath") {
			...
			for(ui i = 0;i < n;i ++) if(pend[peel_sequence[i]]-pstart[peel_sequence[i]] >= K-1) {
				...
				else total_cnt += pm->pivotor2(subgraph_n, matrix, degree, rid, ids, K-1);
			}
			...
			if(sample_n > 0) sample_size = sample_n;
			while(!sample_nodes.empty()&&success_trails < gamma1) {
				ul n_cliques, n_samples;
				printf("expected sample size: %llu\n", sample_size);
				fflush(stdout);
				sample_a_batch(sample_nodes, ub.data(), sample_size, n_cliques, n_samples, ids, out_mapping, local_color, pend, K);
			...
			for(ui i = 0;i < n;i ++) if(pend[peel_sequence[i]]-pstart[peel_sequence[i]] >= K-1) {
				...
				process_a_subgraph(u, subgraph_n, ids_n, ids, out_mapping, K-1, pm, local_color, true,
						total_sample_n, total_sample_time, 0, density_thresholds, ids2.data(), 0.0);
			}
			...
			for(ui i = 0;i < density_thresholds.size()&&!terminate;i ++) {
				...
				if(total_ub < epsilon*(total_cnt + total_ub)) break;
				...
				if(refinement_cap > 0&&refinement_n >= refinement_cap) break;

				vector<ui> &hh = heaps[i];
				ui cc = 0;
				while(!hh.empty()) {
					if(refinement_cap > 0&&refinement_n >= refinement_cap) break;
					if(refinement_cap == 0&&cc%1 == 0&&(i+1 == density_thresholds.size()||expected_sampling_time*2 < t.elapsed())) {
					...
					for(ui j = 0;j < ids_n;j ++) {
						...
						process_a_subgraph(n, subgraph_n, peel_sequence_n, peel_sequence, out_mapping, l-1, pm, local_color,
								false, total_sample_n, total_sample_time, i, density_thresholds, ids2.data(), density);
					}
				}
			...
			printf("*** Preprocessing2 time: %s\n", Utility::integer_to_string(t.elapsed()).c_str());
			printf("*** number of refinements: %u\n", refinement_n);
			fflush(stdout);
			...
			for(ui i = 0;i < density_thresholds.size();i ++) if(!heaps[i].empty()) {
				...
				while(success_trails < gamma1) {
					for(ui j = 0;j < nn;j ++) cnt[j] = 0;
					std::uniform_real_distribution<double> unif(0, nn);
					for(ul j = 0;j < sample_size;j ++) {
						double r = unif(gen);
						ui idx = ui(r);
					...
					for(ui j = i;j < density_thresholds.size();j ++) {
						...
						while(end) {
							...
							if(cnt[cc]) {
								...
								for(ul k = 0;k < cnt[cc];k ++) n_cliques += sample_once_opt(subgraph_n, peel_sequence_n, order_by_color, local_color, l);
							}
...
void Graph::sample_a_batch(vector<ui> &nodes, double *ub, ul sample_size, ul &n_cliques, ul &n_samples,
	...
	for(ui i = 0;i < nodes.size();i ++) {
		...
		construct_DP_table_color_path(subgraph_n, peel_sequence_n, peel_sequence, K-1);

		for(ul j = 0;j < samples;j ++) {
			n_cliques += sample_once(subgraph_n, peel_sequence_n, peel_sequence, local_color, K-1);
		}
		n_samples += samples;
	}
}
...
void Graph::process_a_subgraph(ui leading_vertex, ui subgraph_n, ui peel_sequence_n, ui *peel_sequence, ui *out_mapping, ui l, Pivotor_Matrix *pm,
		ui *local_color, bool record_time, ul &total_sample_n, ul &total_sample_time, ui start_idx, vector<double> &density_thresholds, ui *ids, double dd) {
#ifndef NDEBUG
